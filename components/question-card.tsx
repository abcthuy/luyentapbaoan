...
function StoryListener({
    ...
    useEffect(() => {
        ...
        const loadVoices = () => {
            ...
            const en = all.filter((voice) => voice.lang.startsWith('en'));
            if (en.length > 0) {
                setVoices(en);
                const preferredIndex = en.findIndex(
                    (voice) =>
                        voice.name.includes('Google') ||
                        voice.name.includes('Female') ||
                        voice.name.includes('Samantha')
                );
                if (preferredIndex >= 0) setSelectedVoiceIdx(preferredIndex);
            }
        };
        loadVoices();
    ...
    }, [onReady, selectedVoiceIdx, speed, storyText, voices]);
                ...
                {hasListened && questionText && (
                    ...
                    >
                        <div className="text-2xl md:text-3xl font-black text-indigo-700 text-center px-4">
                            {normalizeDisplayText(questionText)}
                        </div>
                    </motion.div>
                )}
            </motion.div>
...
function parseStoryText(fullText: string): { storyText: string; questionText: string } | null {
    ...
    if (!found) {
        const storyMatch = fullText.match(/(?:📖|ðŸ“–)?\s*Story:\s*\n?"([^"]+)"/);
        if (storyMatch) {
    ...
    storyPart = storyPart
        .replace(/(?:📖|ðŸ“–)?\s*Story:\s*/i, '')
        .replace(/^[""\n]+|[""\n]+$/g, '')
        .trim();

    questionPart = questionPart.replace(/\(Type your answer\)/i, '').trim();
...
export function QuestionCard({
    ...
    const parsedStory = isStoryQuest
        ? parseStoryText(question.content.text || '')
        : isListening
          ? { storyText: question.content.audio || question.content.text || '', questionText: '' }
          : null;
                ...
                <div className="flex flex-col items-center justify-center w-full">
                    {question.instruction && (
                        <div className="text-sm md:text-lg font-black mb-6 text-center text-slate-400 uppercase tracking-widest border-b-2 border-slate-100 pb-2">
                            {normalizeDisplayText(question.instruction)}
                        </div>
                    )}
                    ...
                    {showTTSListener && parsedStory ? (
                        ...
                        (question.type as string) !== 'speaking' &&
                        (question.type as string) !== 'reading' && (
                            <div
                                className={`font-black mb-10 text-center text-indigo-700 leading-snug w-full px-4 whitespace-pre-line ${
                                    (question.content.text?.length || 0) > 200
                                        ? 'text-lg md:text-xl text-left font-bold'
                                        : (question.content.text?.length || 0) > 50
                                          ? 'text-2xl md:text-4xl text-left md:text-center'
                                          : 'text-4xl md:text-6xl'
                                }`}
                            >
                                {normalizeDisplayText(question.content.text)}
                            </div>
                        )
                    )}
                </div>
                                        ...
                                        className={`rounded-[32px] border-4 p-6 text-xl md:text-3xl font-black transition-all transform hover:-translate-y-1 hover:shadow-lg active:translate-y-0 ${
                                            feedback && opt === question.answer
                                                ...
                                                : feedback && opt === answer && opt !== question.answer
                                                  ? 'border-rose-500 bg-rose-100 text-rose-700 shadow-rose-200'
                                                  : 'border-slate-100 bg-slate-50 text-slate-700 hover:border-blue-400 hover:bg-white'
                                        }`}
                                    >
                                        {normalizeDisplayText(opt)}
                                    </button>
                                ))}
                            </div>
